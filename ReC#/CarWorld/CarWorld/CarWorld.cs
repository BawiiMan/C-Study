...
namespace CarWorld
{
    ...
    class Car : IStandard
    {
        ...
        private readonly int _Length;   //일기 전용 필드
        #endregion
        ...
        {
            this.Name = "좋은차";
            _Length = length;   //일기 전용 필드는 생성자로 초기화 가능
            names = new string[length]; //넘어온 값으로 생성
        }
        #endregion
        ...
        public delegate void EventHanbdler();   //대리자: 다중 메서드 호출
        #endregion

        #region [12] 이벤트: Public Events
        public event EventHandler Click;    //이벤트

        #region [13] 이벤트 처리기: EvnetHandlers
    ...
    class CarWorld
    {
        static void Main(string[] args)
        {
            ...
            btn.Click += new Car.EventHanbdler(btn.Run);
            btn.Click += new Car.EventHanbdler(btn.Run);
            btn.OnClick();
        }
    }
